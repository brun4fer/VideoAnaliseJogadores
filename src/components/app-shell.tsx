...
const links = [
  ...
  { href: "/structure", label: "Squad", icon: UsersRound, area: "squad" as const },
];

type Account = { name: string; teamName: string | null; accessControl: { globalUnlocked: boolean; unlockedAreas: AccessArea[] } };
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  function isUnlocked(next: Account, area: AccessArea | null) {
    return !area || next.accessControl.globalUnlocked || next.accessControl.unlockedAreas.includes(area);
  }

  useEffect(() => {
    if (!isPublic) apiFetch<Account>("/api/account").then((next) => {
      setAccount(next);
      if (!isUnlocked(next, currentArea)) {
