...
import { Activity, Archive, CheckCircle2, Download, Film, Loader2, Pencil, Play, Trash2, Upload, UserRound, XCircle } from "lucide-react";
...
export function ReportsClient() {
  ...
  async function downloadFullMatch() {
    const match = data.matches.find((item) => item.id === matchId);
    if (!match) return setError("Select one match before downloading its full video.");
    setDownloadingMatchId(match.id);
    setError(null);
    try {
      const local = files.current.get(match.id) || await getRememberedMatchVideo(match.id).catch(() => null);
      if (local) {
        downloadBlob(local, match.video?.fileName || local.name);
        return;
      }
      if (match.video?.storageStatus !== "READY") throw new Error("This match video is not available in the cloud. Select its local fallback first.");
      const remote = await getRemoteVideoDownloadUrl(match.id);
      const link = document.createElement("a");
      link.href = remote.url;
      link.download = match.video.fileName;
      link.rel = "noopener";
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "The full match video could not be downloaded.");
    } finally {
  ...
  return <div className="space-y-5"><input ref={videoInput} type="file" accept="video/*" multiple className="hidden" onChange={(event) => void addVideos(event.target.files)}/><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><Label>Aggregated analysis</Label><h1 className="mt-2 text-3xl font-bold text-white">Reports and clips</h1><p className="mt-2 text-sm text-slate-400">Filter actions, export clips and download the complete video for a selected match.</p></div><div className="flex flex-wrap gap-2"><Button disabled={matchId === "all" || Boolean(downloadingMatchId)} title={matchId === "all" ? "Select one match first" : "Download the complete match video"} onClick={() => void downloadFullMatch()}>{downloadingMatchId ? <Loader2 size={15} className="animate-spin"/> : <Download size={15}/>}Download full match</Button><Button onClick={() => videoInput.current?.click()}><Upload size={15}/>Local fallback</Button></div></div>{error ? <p className="rounded-lg border border-red-400/20 bg-red-400/10 p-3 text-sm text-red-200">{error}</p> : null}{exporting ? <p className="rounded-lg border border-cyan-300/20 bg-cyan-300/10 p-3 text-sm text-cyan-100">{status}</p> : null}
    ...
    <div data-video-workspace className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_24rem]">
      <ActionClipPlayer key={playbackRequest} area="reports" action={selected} onClipEnd={selectedIndex >= 0 && selectedIndex < filtered.length - 1 ? () => setSelectedId(filtered[selectedIndex + 1].id) : undefined} onEdit={selected ? () => editAction(selected) : undefined} onDelete={selected ? () => void deleteAction(selected) : undefined} deleting={deletingId === selected?.id}/>
