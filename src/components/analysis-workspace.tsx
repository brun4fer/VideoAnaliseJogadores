...
import { Archive, ArrowLeft, ChevronLeft, ChevronRight, Clock3, Cloud, FileVideo, FolderOpen, GripVertical, Loader2, Pause, Pencil, Play, RefreshCw, Tags, Trash2, Upload, UserRound, X } from "lucide-react";
...
export function AnalysisWorkspace({ matchId }: { matchId: string }) {
  ...
  async function openCloudLibrary() {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Could not open the cloud video library.";
      setCloudLibraryError(message);
      setNotice(message);
    } finally {
...
function VideoPanel({ sourceUrl, videoRef, duration, currentTime, rate, playing, previewEnd, lastAction, match, currentPeriod, setDuration, setCurrentTime, setPlaying, setPreviewEnd, setRate, seekTo, onChoose, onDeleteLast, onSetPeriodMarker, identificationControl, onEnded }: { sourceUrl: string | null; videoRef: RefObject<HTMLVideoElement | null>; duration: number; currentTime: number; rate: number; playing: boolean; previewEnd: number | null; lastAction: ActionRecord | null; match: MatchDetail; currentPeriod: number | null; setDuration: (value: number) => void; setCurrentTime: (value: number) => void; setPlaying: (value: boolean) => void; setPreviewEnd: (value: number | null) => void; setRate: (value: number) => void; seekTo: (value: number) => void; onChoose: () => void; onDeleteLast: () => void; onSetPeriodMarker: (key: PeriodMarkerKey) => void; identificationControl: React.ReactNode; onEnded: () => void }) {
  return <Panel className="flex min-h-0 flex-col overflow-hidden md:h-[calc(100dvh-10rem)]">
    <div className="relative aspect-video min-h-0 bg-black md:flex-1 md:aspect-auto">{sourceUrl ? <video ref={videoRef} src={sourceUrl} crossOrigin="anonymous" className="h-full w-full object-contain" playsInline onLoadedMetadata={(event) => { setDuration(event.currentTarget.duration); event.currentTarget.playbackRate = rate; }} onTimeUpdate={(event) => { setCurrentTime(event.currentTarget.currentTime); if (previewEnd !== null && event.currentTarget.currentTime >= previewEnd - .04) { event.currentTarget.pause(); event.currentTarget.currentTime = previewEnd; setPreviewEnd(null); } }} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onEnded={onEnded}/> : <button onClick={onChoose} className="flex h-full w-full flex-col items-center justify-center p-6 text-center"><FileVideo size={38} className="text-cyan-300"/><h2 className="mt-3 text-sm font-bold text-white">Upload the match video</h2><p className="mt-1 text-xs text-slate-500">It will be stored privately in Cloudflare R2.</p></button>}</div>
    <div className="shrink-0 border-t border-white/10 bg-pitch-950/95 p-2">
      ...
      <div className="mt-1.5 flex items-center gap-2">
        ...
        <div className="flex shrink-0 items-center gap-2"><span className="hidden items-center gap-1 font-mono text-xs text-white sm:inline-flex"><Clock3 size={13} className="text-cyan-300"/>{formatTime(currentTime)} / {formatTime(duration)}</span>{identificationControl}</div>
      </div>
    </div>
  </Panel>;
}

function RecordedOccurrences({ actions, players, sideStyle, filterPlayerId, exporting, onFilter, onPreview, onEdit, onDelete, onExport }: { actions: ActionRecord[]; players: PlayerRecord[]; sideStyle?: CSSProperties; filterPlayerId: string; exporting: boolean; onFilter: (id: string) => void; onPreview: (action: ActionRecord) => void; onEdit: (action: ActionRecord) => void; onDelete: (action: ActionRecord) => void; onExport: () => void }) {
  return <Panel style={sideStyle} className="flex min-h-0 flex-col overflow-hidden xl:h-[var(--analysis-side-height)]"><div className="shrink-0 border-b border-white/10 p-2"><div className="flex items-center justify-between gap-2"><div><Label>Recorded actions</Label><p className="text-[10px] text-slate-500">{actions.length} occurrences</p></div><select aria-label="Filter by player" value={filterPlayerId} onChange={(event) => onFilter(event.target.value)} className="min-w-0 max-w-28 rounded-md border border-white/10 bg-pitch-900 p-1.5 text-[10px]"><option value="all">All</option>{players.map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}</select></div><Button size="sm" className="mt-2 h-7 w-full text-[10px]" disabled={!actions.length || exporting} onClick={onExport}>{exporting ? <Loader2 size={12} className="animate-spin"/> : <Archive size={12}/>}Export {filterPlayerId === "all" ? "all clips" : "player clips"}</Button></div><div className="min-h-0 flex-1 overflow-y-auto">{actions.length ? actions.map((action, index) => { const first = action.subActions[0]; const type = first ? actionTypeByKey.get(first.actionKey) : null; const color = actionResultColor(first?.outcome || type?.outcome); const outcome = first?.outcome || type?.outcome || "neutral"; return <div key={action.id} className="flex h-10 items-center gap-1.5 border-b border-white/[.06] px-2" title={action.subActions.map((item) => item.actionName).join(" · ") || "Unclassified moment"}><button onClick={() => onPreview(action)} className="flex min-w-0 flex-1 items-center gap-1.5 text-left"><span className="w-4 shrink-0 text-right font-mono text-[9px] text-slate-600">{index + 1}</span><span className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: color }}/><span className="min-w-0 flex-1 truncate text-[10px] font-semibold text-white">{action.player.name}</span><span className="shrink-0 font-mono text-[9px] text-slate-500">{formatTime(action.startTimeSeconds)}–{formatTime(action.endTimeSeconds)}</span><Badge className="shrink-0 px-1 py-0 text-[8px]">{action.subActions.length}</Badge><span className="sr-only">{outcome}</span></button><button type="button" aria-label={`Edit clip ${index + 1}`} title="Edit clip" onClick={() => onEdit(action)} className="p-1 text-slate-500 hover:text-cyan-200"><Pencil size={11}/></button><button type="button" aria-label={`Delete clip ${index + 1}`} title="Delete clip" onClick={() => void onDelete(action)} className="p-1 text-slate-600 hover:text-red-300"><Trash2 size={11}/></button></div>; }) : <p className="p-4 text-center text-xs text-slate-500">No occurrences yet.</p>}</div></Panel>;
}
...
function Timeline({ players, actions, duration, onSelect }: { players: PlayerRecord[]; actions: ActionRecord[]; duration: number; onSelect: (action: ActionRecord) => void }) {
  ...
  return <Panel className="p-2"><div className="flex items-center justify-between"><div><Label>Timeline</Label><span className="ml-2 text-[10px] text-slate-500">One row per player</span></div><span className="text-[10px] text-slate-500">−4s / +6s</span></div><div className="mt-2 overflow-x-auto"><div className="min-w-[620px] overflow-hidden rounded-md border border-white/10">{visible.length ? visible.map((player) => <div key={player.id} className="grid grid-cols-[8rem_minmax(0,1fr)] border-b border-white/[.07] last:border-0"><div className="truncate border-r border-white/[.07] px-2 py-1.5 text-[10px] text-slate-300">{player.name}</div><div className="relative min-h-7 bg-black/20">{actions.filter((action) => action.playerId === player.id).map((action) => { const left = Math.max(0, (action.startTimeSeconds / duration) * 100); const width = Math.max(.6, ((action.endTimeSeconds - action.startTimeSeconds) / duration) * 100); const first = action.subActions[0]; const type = first ? actionTypeByKey.get(first.actionKey) : null; const color = actionResultColor(first?.outcome || type?.outcome); return <button key={action.id} title={`${action.subActions.map((item) => item.actionName).join(", ") || "Unclassified"} · ${formatTime(action.eventTimeSeconds)}`} onClick={() => onSelect(action)} className="absolute top-1/2 h-3 -translate-y-1/2 rounded-full border border-white/25 hover:h-5" style={{ left: `${left}%`, width: `${width}%`, backgroundColor: color }}/>; })}</div></div>) : <p className="p-3 text-xs text-slate-500">No occurrences yet.</p>}</div></div></Panel>;
}
