...
export function ActionClipPlayer({ action, className = "", onEdit, onDelete, onClipEnd, deleting = false }: { action: ClipAction | null; className?: string; onEdit?: () => void; onDelete?: () => void; onClipEnd?: () => void; deleting?: boolean }) {
  ...
  return <Panel className={`overflow-hidden ${className}`}>
    ...
    <div className="relative aspect-video bg-black">{sourceUrl && action ? <video key={`${sourceUrl}-${action.id}`} src={sourceUrl} crossOrigin="anonymous" controls playsInline className="h-full w-full object-contain" onLoadedMetadata={(event) => { const end = Math.min(event.currentTarget.duration, action.endTimeSeconds); event.currentTarget.currentTime = Math.min(action.startTimeSeconds, end); void event.currentTarget.play(); }} onPlay={(event) => { const end = Math.min(event.currentTarget.duration, action.endTimeSeconds); if (event.currentTarget.currentTime < action.startTimeSeconds || event.currentTarget.currentTime >= end) { completedActionRef.current = null; event.currentTarget.currentTime = Math.min(action.startTimeSeconds, end); } }} onTimeUpdate={(event) => { const end = Math.min(event.currentTarget.duration, action.endTimeSeconds); if (event.currentTarget.currentTime >= end && completedActionRef.current !== action.id) { completedActionRef.current = action.id; event.currentTarget.pause(); event.currentTarget.currentTime = end; onClipEnd?.(); } }}/> : <div className="flex h-full flex-col items-center justify-center p-5 text-center"><FileVideo size={32} className="text-cyan-300"/><p className="mt-3 text-xs text-slate-400">{action ? loading ? "Restoring video…" : notice || "Upload this match’s video in its analysis page." : "Select an action to view its clip."}</p>{action && !loading ? <Button size="sm" className="mt-3" onClick={() => fileRef.current?.click()}><Upload size={14}/>Use local fallback</Button> : null}</div>}</div>
  </Panel>;
}
