...
type Player = { id: string; name: string; shirtNumber: number | null; photoUrl: string | null; position: string | null; isGoalkeeper: boolean; footballSyncedAt: string | null };
...
export function StructureClient() {
  ...
  async function synchronize(kind: "season" | "competition" | "team" | "player" | "opponent", id: string, label: string, competitionId?: string) {
    const key = `${kind}:${id}`;
    setSyncing(key); setNotice(null);
    try {
      await apiFetch("/api/football-sync", { method: "POST", body: JSON.stringify({ kind, id, competitionId }) });
      await load();
  ...
  return <div className="space-y-5">
    ...
    <Panel className="p-4"><div className="flex items-center justify-between"><div><Label>Active team squad</Label><h2 className="mt-1 text-xl font-bold text-white">{data.clientClub?.name || "Set up the team first"}</h2></div><UserRound className="text-cyan-300"/></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">{data.clientClub?.players?.map((player) => <div key={player.id} className="flex min-h-20 items-center gap-2 rounded-lg border border-white/10 bg-black/15 p-2.5"><PlayerPhoto player={player}/><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-white">{player.shirtNumber ? `${player.shirtNumber} · ` : ""}{player.name}</p><p className="truncate text-xs text-slate-500">{playerPositionLabel(player.position)}</p><button type="button" disabled={Boolean(syncing)} title={player.footballSyncedAt ? `Synchronized ${new Date(player.footballSyncedAt).toLocaleString()}. Click to synchronize again.` : `Synchronize ${player.name} with FootballOurPlayers`} onClick={() => void synchronize("player", player.id, player.name)} className={`mt-1 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[9px] font-semibold transition disabled:opacity-40 ${player.footballSyncedAt ? "bg-emerald-400/10 text-emerald-300" : "bg-cyan-300/10 text-cyan-300"}`}>{syncing === `player:${player.id}` ? <Loader2 size={10} className="animate-spin"/> : player.footballSyncedAt ? <Check size={10}/> : <RefreshCw size={10}/>} {player.footballSyncedAt ? "Synced" : "Sync"}</button></div><ImageUploadButton label={`Replace photo for ${player.name}`} disabled={busy} onSelect={(file) => void replaceImage("players", player.id, file)}/><button type="button" aria-label="Delete player" onClick={() => void remove("player", player.id, player.name)} className="text-slate-600 hover:text-red-300"><Trash2 size={14}/></button></div>)}</div></Panel>
  </div>;
}
