...
import { CalendarDays, Check, ChevronRight, Film, Pencil, Plus, RefreshCw, Trash2, UsersRound, X } from "lucide-react";
...
type Match = { id: string; matchDate: string | null; roundName: string | null; venue: string | null; notes: string | null; firstHalfAttacksRight: boolean; footballSyncedAt: string | null; competitionId: string; opponentClubId: string; club: { name: string }; opponentClub: { name: string }; competition: { name: string; season: { name: string } }; video: { storageStatus: "LOCAL" | "UPLOADING" | "READY" | "FAILED" } | null; _count: { playerActions: number; squad: number } };
...
export function MatchesDashboard() {
  ...
  return <div className="space-y-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><Label>Analysis centre</Label><h1 className="mt-1 text-2xl font-bold text-white">Matches</h1><p className="mt-1 text-sm text-slate-400">Select a match to analyse individual player actions.</p></div><Link href="/matches/new"><Button variant="primary"><Plus size={16}/>New match</Button></Link></div>
    ...
    <div className="grid gap-3 lg:grid-cols-2 2xl:grid-cols-3">{matches.map((match) => { const cloudReady = match.video?.storageStatus === "READY"; const videoLabel = cloudReady ? "Video in cloud" : match.video?.storageStatus === "UPLOADING" ? "Upload incomplete" : match.video ? "Needs upload" : "No video"; return <Panel key={match.id} className="group overflow-hidden"><Link href={`/analysis/${match.id}`} className="block p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[10px] font-bold uppercase tracking-[.15em] text-cyan-300">{match.competition.name} · {match.competition.season.name}</p><h2 className="mt-1 truncate text-lg font-bold text-white">{match.club.name} vs {match.opponentClub.name}</h2><p className="text-xs text-slate-400">{match.roundName || "Match"}</p></div><ChevronRight size={18} className="text-slate-600 transition group-hover:translate-x-1 group-hover:text-cyan-300"/></div><div className="mt-3 flex flex-wrap gap-1.5 text-[10px] text-slate-400"><span className="inline-flex items-center gap-1 rounded bg-white/[.05] px-2 py-1"><CalendarDays size={11}/>{match.matchDate ? new Date(match.matchDate).toLocaleDateString("en-GB") : "No date"}</span><span className="rounded bg-white/[.05] px-2 py-1">{match._count.playerActions} occurrences</span><span className="rounded bg-white/[.05] px-2 py-1">{match._count.squad || "Legacy"} players</span><span className={`rounded px-2 py-1 ${cloudReady ? "bg-emerald-400/10 text-emerald-200" : match.video ? "bg-amber-300/10 text-amber-100" : "bg-white/[.05]"}`}>{videoLabel}</span></div></Link><div className="flex items-center justify-between gap-3 border-t border-white/[.06] px-4 py-2"><Link href={`/analysis/${match.id}/sync`} className={`inline-flex items-center gap-1 rounded px-2 py-1 text-xs font-semibold transition ${match.footballSyncedAt ? "bg-emerald-400/10 text-emerald-200 hover:bg-emerald-400/15" : "bg-cyan-300/10 text-cyan-200 hover:bg-cyan-300/15"}`}>{match.footballSyncedAt ? <Check size={13}/> : <RefreshCw size={13}/>}Sync{match.footballSyncedAt ? "ed" : ""}</Link><div className="flex gap-3"><button className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-cyan-300" onClick={() => setEditing(match)}><Pencil size={13}/>Edit</button><button className="inline-flex items-center gap-1 text-xs text-slate-600 hover:text-red-300" onClick={() => void remove(match.id)}><Trash2 size={13}/>Delete</button></div></div></Panel>; })}</div>
