...
export async function DELETE(_: Request, context: { params: Promise<{ subActionId: string }> }) {
  try { const { workspace } = await requireAreaAccount(["analysis", "reports"]); const { subActionId } = await context.params; await prisma.playerSubAction.deleteMany({ where: { id: subActionId, playerAction: { match: { workspaceId: workspace.id } } } }); return ok({ deleted: true }); }
  catch (error) { return serverError(error); }
}
